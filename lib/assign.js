...
export async function pickAssignee(issue) {
  ...
  // Prefer devs under WIP limit, but still assign overloaded if no choice
  const underLimit = candidates.filter((d) => load[d] < (team[d].max_open || 999));
  if (underLimit.length) {
    candidates = underLimit;
  }
  // If no skill match, fall back to all devs (prefer under-limit)
  if (!candidates.length) {
    const allUnder = devs.filter((d) => load[d] < (team[d].max_open || 999));
    candidates = allUnder.length ? allUnder : devs;
  }
  if (!candidates.length) {
...
export async function assignAndAnnounce(issue) {
  ...
  const repoName = (issue.repository_url || "").split("/").pop() || process.env.GITHUB_REPO || "this repo";
  const branchHint = `${issue.number}-${(issue.title || "fix").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^\[.*?\]-?/, "").slice(0, 40)}`;
  await commentIssue(
    issue,
    `**Auto-assigned to @${result.dev}**\n\n_${result.reason}_\n\n**How to work on this:**\n1. Repo: \`${repoName}\`\n2. Create branch: \`${branchHint}\`\n3. Open PR → status auto-moves to **In Review**\n4. Close this issue → **Done**`
  );

  // WIP overload alert: still assign but warn PM
  const team = loadTeam();
  const devConfig = team[result.dev];
  if (devConfig && result.load + 1 > devConfig.max_open) {
    const { postDiscord, makeEmbed } = await import("./discord.js");
    await postDiscord({
      embeds: [makeEmbed({
        title: `WIP ALERT: @${result.dev} now at ${result.load + 1}/${devConfig.max_open}`,
        description: `Assigned [#${issue.number}](${issue.html_url}) **${issue.title}**\n\n@${result.dev} is over WIP limit. PM: consider reassigning or adjusting workload.`,
        color: 0xF59E0B,
      })],
    });
  }
