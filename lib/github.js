...
async function getActiveRepos(org) {
  ...
  try {
    ...
    // All repos updated in last 30 days (no limit)
...
export async function listPulls({ state = "all", days = 30 } = {}) {
  ...
  if (s.mode === "org") {
    ...
    const fetches = repos.map(async (r) => {
      try {
        const pulls = await ghGet(`/repos/${r}/pulls`, { state, sort: "updated", direction: "desc", per_page: 100 });
...
export async function listCommits({ days = 7 } = {}) {
  ...
  if (s.mode === "org") {
    // Fetch from all active repos (no limit), 100 commits each
    const repos = await getActiveRepos(s.org);
    const all = [];
    const fetches = repos.map(async (r) => {
      try {
        const cs = await ghGet(`/repos/${r}/commits`, { since, per_page: 100 });
        all.push(...cs.map((c) => ({ ...c, _repo: r })));
      } catch (e) { /* skip empty/inaccessible repos */ }
    });
    await Promise.all(fetches);
    return all;
  }
